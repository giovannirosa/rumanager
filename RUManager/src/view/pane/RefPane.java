...
public class RefPane extends UtilPane {
	...
	public RefPane(Table<TableModel> table, AbstractTab tab) {
		...
		noSpin.valueProperty().addListener((obs, oldValue, newValue) -> {
			custoField.setText(new DecimalFormat("##.00").format(custo*newValue));
			calorField.setText(Integer.toString(calor*newValue));
		});
	...
	private void removeStorage() {
		for (TableModel m : recTable.getItems()) {
			...
			for (Ing i : rModel.getListaIngrediente()) {
				...
				item.setQtde(item.getQtde()-i.getQtde());
			}
		}
		Main.itemTab.refreshTable();
		Main.itemTab.refreshUITable();
	}
	...
	private boolean checkStorage() {
		for (TableModel m : recTable.getItems()) {
			...
			for (Ing i : rModel.getListaIngrediente()) {
				Item item = (Item) Main.itemControl.get(i.getDesc());
				if (item == null) {
					Factory.showWarning("O ingrediente "+i.getDesc()+" não foi encontrado no estoque, "
							+ "ele pode ter sido deletado ou mudou de nome!\n"
							+ "Por favor verifique o estoque ou atualize a receita!\n");
					return false;
				}
				if (item.getQtde() < i.getQtde()) {
					Factory.showWarning("O ingrediente "+i.getDesc()+" não tem quantidade suficiente no estoque!\n"
							+ "Por favor verifique o estoque ou atualize a receita!\n");
					return false;
				}
			}
